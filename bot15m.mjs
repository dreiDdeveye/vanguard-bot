// ═══════════════════════════════════════════════════════════════
// VANGUARD 15-MINUTE BOT — Runs 24/7 on Render
// Same logic as bot.mjs but for 15-minute BTC windows
// ═══════════════════════════════════════════════════════════════
...
const DISCORD_WEBHOOK_URL = process.env.DISCORD_WEBHOOK_URL_15M || process.env.DISCORD_WEBHOOK_URL || null;

// ── 15-minute specific config ──
const WINDOW_SECS   = 900;   // 15 minutes
const INTERVAL      = '15m'; // Binance candle interval
const SOURCE        = 'vanguard-bot-15m';
const SLUG_PREFIX   = 'btc-updown-15m-';
const LIVE_PRED_ID  = 2;     // id=2 in live_prediction table
const BOT_TAG       = '[BOT-15M]';
...
const PORT = process.env.PORT_15M || process.env.PORT || 3001;
createServer((req, res) => {
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify({
    bot: '15m', status: 'running',
...
async function fetchPTBFallback() {
  ...
  // Last resort: use current live BTC price as PTB
  if (state.btcPrice) {
    console.log(BOT_TAG, 'PTB last resort: using live price', state.btcPrice);
...
async function publishLivePrediction({ direction, ptb, btcPrice, confidence, confPct, bullScore, bearScore, signalsText }) {
  ...
  try {
    ...
    if (!res.ok) console.error(BOT_TAG, 'Live pred publish error:', res.status, await res.text());
...
async function savePrediction(windowStart, ptb, endPrice, predictedOver) {
  ...
  try {
    const res = await fetch(SUPABASE_URL + '/rest/v1/predictions_15m', {
      method: 'POST',
      headers: { ...SB_HEADERS, 'Prefer': 'resolution=merge-duplicates' },
      body: JSON.stringify(row),
    });
...
async function loadStats() {
  try {
    const url = SUPABASE_URL + '/rest/v1/predictions_15m?select=ts,over&source=eq.' + SOURCE + '&order=ts.desc&limit=1000';
...
async function discordPrediction(direction, confLabel, confPct, bullScore, bearScore, signals) {
  ...
  await sendDiscord({
    username: 'Vanguard 15M Bot', avatar_url: 'https://i.imgur.com/AfFp7pu.png',
    embeds: [{
      title: `${isUp ? '🟢' : '🔴'}  BTC 15m Call — ${isUp ? '⬆️ UP' : '⬇️ DOWN'}`,
      ...
      footer: { text: `15M Window: ${windowTime}  •  Vanguard 15M Bot` },
...
async function discordSkip(reason) {
  ...
  await sendDiscord({
    username: 'Vanguard 15M Bot', avatar_url: 'https://i.imgur.com/AfFp7pu.png',
    embeds: [{
      title: '🚫  NO TRADE — 15M SKIP', color: 0x555555,
...
async function makePrediction() {
  ...
  // Signal 3: Window trajectory (analysis snapshot vs now)
  if (state.analysisDone && state.analysisPrice) {
    ...
    if (state.analysisOddsUp != null && state.upPct) {
      const shift = parseFloat(state.upPct) - state.analysisOddsUp;
      if (shift > 3)       { bullScore += 1; signals.push(`ODDS↑ +${shift.toFixed(1)}`); }
      else if (shift < -3) { bearScore += 1; signals.push(`ODDS↓ ${shift.toFixed(1)}`); }
    }
  }

  // Signal 4: 15m candle momentum (last 3 candles)
  try {
    const candles = await fetchCandles(INTERVAL, 5);
    if (candles && candles.length >= 3) {
      const recent = candles.slice(-3);
      const momPct = ((recent[recent.length-1].close - recent[0].open) / recent[0].open) * 100;
      if (momPct > 0.05)       { bullScore += 1.5; signals.push(`MOM↑ +${momPct.toFixed(3)}%`); }
      else if (momPct < -0.05) { bearScore += 1.5; signals.push(`MOM↓ ${momPct.toFixed(3)}%`); }
      else if (momPct > 0.02)  { bullScore += 0.5; signals.push(`MOM↑ +${momPct.toFixed(3)}%`); }
      else if (momPct < -0.02) { bearScore += 0.5; signals.push(`MOM↓ ${momPct.toFixed(3)}%`); }
    }
  } catch(e) {}
  ...
  let isUp = bullScore >= bearScore ? diff >= 0 : bullScore > bearScore;
  if (bullScore !== bearScore) isUp = bullScore > bearScore;
...
async function tick() {
  ...
  // New window — save previous prediction
  if (state.currentWindowStart !== 0 && state.currentWindowStart !== windowStart) {
    console.log(BOT_TAG, `New window detected. predictionMade=${state.predictionMade} dir=${state.predictionDirection} ptb=${state.predictionPTB} price=${state.btcPrice}`);
    ...
    } else if (state.predictionMade && !state.predictionDirection) {
      console.log(BOT_TAG, 'Skipped save — NO TRADE window (skip already saved)');
    } else {
      console.log(BOT_TAG, 'WARNING: prediction not saved — missing fields');
    }
    ...
    console.log(BOT_TAG, `\n═══ NEW 15M WINDOW: ${new Date(windowStart * 1000).toLocaleTimeString()} ═══`);
  ...
  if (!state.priceToBeat) {
    ...
    if (!state.priceToBeat) {
      try {
        ...
        if (ptb) { state.priceToBeat = ptb; state.ptbSource = 'binance'; console.log(BOT_TAG, 'PTB from Binance fallback:', ptb); }
      } catch(e) {}
    }
  } else {
  ...
  // Analysis snapshot at 70% of window elapsed (30% remaining)
  const analysisMark = Math.floor(WINDOW_SECS * 0.30);
  if (!state.analysisDone && timeLeft <= analysisMark + 30 && timeLeft > analysisMark && state.priceToBeat && state.btcPrice) {
    ...
    console.log(BOT_TAG, `ANALYSIS SNAPSHOT | BTC: $${state.btcPrice.toFixed(2)} | PTB dist: ${((state.btcPrice - state.priceToBeat)/state.priceToBeat*100).toFixed(3)}%`);
  }

  // Place vote at 25% remaining
  const voteMark = Math.floor(WINDOW_SECS * 0.25);
...
async function init() {
  console.log(BOT_TAG, '═══════════════════════════════════════');
  console.log(BOT_TAG, 'VANGUARD 15-MINUTE BOT STARTING');
  console.log(BOT_TAG, '═══════════════════════════════════════');
  await loadStats();
  connectRTDS();
